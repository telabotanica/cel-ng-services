...
final class CelSyncGetJobsCommand extends Command
{
    ...
    protected function configure()
    {
        $this
			...
			->addOption('yesterdayAM', null, InputOption::VALUE_NONE,
                'If set, get jobs from yesterday')
			->addOption('yesterdayPM', null, InputOption::VALUE_NONE,
                'If set, get jobs from yesterday')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
		...
		// Adding yesterdayAM & yesterday PM in order to run 2 processes at the same time to speed up the script
        $yesterdayAM = $input->getOption('yesterdayAM');
        $yesterdayPM = $input->getOption('yesterdayPM');
		...
		if ($yesterday || $yesterdayAM || $yesterdayPM){
			...
			if (!$yesterdayPM){
				...
				$startDateYesterday = $datePrecedente->setTime(12, 0, 0);
			}
			...
			if (!$yesterdayAM){
				...
				$endDateYesterday = (clone $datePrecedente)->setTime(11, 59, 59);
			}
						...
						*/
						...
						if ($existingOccurrence) {
							if ($occurrence->isDeleted() || $occurrence->isCensored()) {
								...
								if ($previousSciNameId != $newSciNameId || $imageChanged || $geoChanged) {
...
//        }
		...
		$startDateInSeconds = $startDate / 1000;
		$startDateForInfo = (new \DateTime())->setTimestamp($startDateInSeconds);
		$startDateForInfo = $startDateForInfo->format('d/m/Y H:i');
		
		$endDateInSeconds = $endDate / 1000;
		$endDateForInfo = (new \DateTime())->setTimestamp($endDateInSeconds);
		$endDateForInfo = $endDateForInfo->format('d/m/Y H:i');
		
		$this->io->success([
			sprintf('Success! Got %d new jobs, %d already know, out of %d total processed occurrences!',
			$this->newJobsCount, $this->existingJobsCount, count($occurrences)
		),
		   sprintf('Job started at %s, Job finished at %s, get datas from %s to %s',$timeStarted, $timeFinished, $startDateForInfo, $endDateForInfo)
		]);
		
		if ($email){
			$this->io->success(sprintf('get datas from user %s', $email));
		}
