...
CREATE TABLE `change_log` (
  ...
  `error_count` tinyint(2) DEFAULT NULL COMMENT 'Nombre d''échecs lors du traitement de ce changeLog"',
  PRIMARY KEY (`id`)
...
CREATE TABLE `extended_field` (
  ...
  `is_visible` tinyint(1) DEFAULT 1 COMMENT 'Champ invisible de l''utilisateur mais nécessaire au projet\n     ',
  `is_mandatory` tinyint(1)  DEFAULT 0 COMMENT 'Indique si le champ est obligatoire pour envoyer la donnée ou non',
...
CREATE TABLE `project_settings` (
  ...
  `language` varchar(2) COLLATE utf8mb4_unicode_ci DEFAULT 'FR' COMMENT 'Langue du projet',
  ...
  `published_location` enum('précise','localité','10x10km') COLLATE utf8mb4_unicode_ci DEFAULT '10x10km' COMMENT 'Précision géographique à laquelle est publiée l''obs, permet de gérer le floutage(DC2Type:publishedlocationenum)',
   basemap varchar(10) COLLATE utf8mb4_unicode_ci DEFAULT 'OSM' COMMENT 'Précision du fond de carte affiché lors de la saisie',
  `is_mandatory_address` tinyint(1) DEFAULT 0 COMMENT 'Indique si l adresse doit être stockée ou non',
  `is_mandatory_photo` tinyint(1) DEFAULT 0 COMMENT 'Indique si la photo doit être stockée ou non',
